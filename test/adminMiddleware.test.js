...
test('admin identity accepts camelCase Monday session token payloads', () => {
  assert.deepEqual(
    ...
    { accountId: '1001', userId: '2002', isViewOnly: false }
  );
});
...
test('admin identity captures Monday view-only users', () => {
  assert.deepEqual(
    getAdminIdentity({ data: { account_id: 1001, user_id: 2002, isViewOnly: true } }),
    { accountId: '1001', userId: '2002', isViewOnly: true }
  );
});
